...
public class WriteExcelSbsj {
	public void getExcel(String sheetName, OutputStream output,String nsrwjbm,String startdate,String enddate,String swjgbm) {
		...
		strSql = sqlNsr+" and a.NSRWJBM in(select NSRWJBM from SKQ_NSRXX where STATUS=1 "+sqlSwjg+")";
		...
		ArrayList al = cx.selectSbsjExcel(strSql);
	...
	public void getExcel1(String sheetName, OutputStream output,String nsrwjbm,String startdate,String enddate) {
		String strSql = "";
		String sqlSwjg = "";
		String sqlNsr = "";
		if(nsrwjbm!=null&&!"".equals(nsrwjbm)){
		...
		try {
			File file=new File("d:\\"+Util.getPreviousMonthFirst().substring(0,7)+"申报汇总");
			if(!file.isDirectory()){
				file.mkdirs(); 
			}
		 FileOutputStream fileOut = new FileOutputStream("d:\\"+Util.getPreviousMonthFirst().substring(0,7)+"申报汇总"+"\\"+sheetName);
		 fileOut.flush();
		 wb.write(fileOut);
		 fileOut.close();
		} catch (IOException e) {
