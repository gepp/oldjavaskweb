...
String jqszsmStr = jqxx.getJqszsmStr();
...
function get_id(){
	var s='';
	var length = document.all.getElementById('form1').elements.length;
...
function add(){
	...
	var num = document.all.getElementById('num').value;
	if(num==0){
		alert('请选择机器对应税种税目！');
	}
	else if(num>6){
		...
		if(validator(frm)){
			...
			frm.action='/javaskweb/taxpayerServlet?method=skzzxxUpdate';
			frm.submit();
		}
	}
}
...
%>
              <option value="<%=jqxh.getJqxhbm() %>" <% if(jqxx.getJqxhbm().equals(jqxh.getJqxhbm())){out.println("selected");} %>><%=jqxh.getJqxhmc() %></option>
              <%
	}
}
%>
          ...
          <td align="center"><input id="jqbh" name="jqbh" type="text" maxlength="16" value="<%=jqxx.getJqbh() %>" required="required" dataType="integer" controlName="机器编号" />
            &nbsp;<font color="#FF0000">*</font></td>
          <td align="center"><input id="skkh" name="skkh" type="text" maxlength="16" value="<%=jqxx.getSkkh() %>" required="required" dataType="integer" controlName="税控卡号" />
            &nbsp;<font color="#FF0000">*</font></td>
          <td align="center">&nbsp;</td>
        </tr>
      </table>
...
%>
        <tr>
          ...
          <td align="center"><input type="checkbox" name="a<%=(String)hm.get("smbm") %>" id="a<%=(String)hm.get("smbm") %>" value="<%=(String)hm.get("smbm") %>" onclick="checkPZ('<%=(String)hm.get("fpbm") %>',this)" <% if(jqszsmStr.indexOf((String)hm.get("smbm"))>=0){out.println("checked");} %> /></td>
        </tr>
        <%
	}
}
%>
      ...
      <table width="100%" border="0" cellspacing="0" cellpadding="0" id="tableListBottom">
        <tr>
          <td height="40" align="center"><input type="hidden" name="nsrwjbm" value="<%=nsrwjbm %>"/>
            <input type="hidden" id="jumpFlag" name="jumpFlag" value="<%=jumpFlag %>"/>
            <input type="hidden" id="oldJqbh" name="oldJqbh" value="<%=jqxx.getJqbh() %>"/>
            <input type="hidden" id="oldSkkh" name="oldSkkh" value="<%=jqxx.getSkkh() %>"/>
            <input type="hidden" id="jqszsmStr" name="jqszsmStr" value="<%=jqxx.getJqszsmStr() %>"/>
            <input type="hidden" name="pzbm" id="pzbm" value="<%=jqxx.getFpbm() %>"/>
            <input type="hidden" name="num" id="num" value="<%=jqszsm.size() %>"/>
