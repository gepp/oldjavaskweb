...
<div id="right">
  ...
  <div id="main">
	...
	id="userList">
      ...
      <tr>
        <td>单笔开票最高限额（元）：</td>
        <td><font color="#0000FF"><strong><%=jqxx.getDzkpxe() %></strong></font>&nbsp;</td>
        <td>月累计开票最高限额（元）：</td>
        <td><font color="#0000FF"><strong><%=jqxx.getYljkpxe() %></strong></font>&nbsp;</td>
      </tr>
      <tr>
        <td>月累计退票最高限额（元）：</td>
        <td><font color="#0000FF"><strong><%=jqxx.getYljtpxe() %></strong></font>&nbsp;</td>
        <td>&nbsp;</td>
        <td>&nbsp;</td>
      </tr>
      <tr>
        <td width="20%">纳税人微机编码：</td>
        <td width="30%"><%=nsrxx.getNsrwjbm() %>&nbsp;</td>
        <td width="20%">纳税人名称：</td>
        <td width="30%"><%=nsrxx.getNsrmc() %>&nbsp;</td>
      </tr>
    </table>
    <br />
    <table width="100%" border="0" cellspacing="0" cellpadding="0"
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0" id="tableListBottom">
      <tr>
        <td height="50" align="center"><!-- 纳税人微机代码-->
          <input type="hidden" name="TAXPAYER_NO" value="<%=("0"+nsrxx.getNsrwjbm()) %>" />
          <!-- 税控卡号-->
          <input type="hidden" name="FISCAL_CARD_NO" value="<%=jqxx.getSkkh() %>" />
          <!-- 机器编号-->
          <input type="hidden" name="MACHINE_NO" value="<%=jqxx.getJqbh() %>" />
          <!-- 用户卡有效时间-->
          <input type="hidden" name="U_START_DATE" value="<%=Util.toxkrq(jqxx.getKqyrq()) %>" />
          <!-- 用户卡截止时间-->
          <input type="hidden" name="U_END_DATE" value="<%=Util.toxkrq(jqxx.getKyxrq()) %>" />
          <!-- 纳税户名称-->
          <input type="hidden" name="TAXPAYER_NAME" value="<%=nsrxx.getNsrmc() %>" />
          <!-- 申报截止时间-->
          <input type="hidden" name="MAKE_END_DATE" value="<%=Util.toxkrq(jqxx.getKpjzrq()) %>" />
          <!-- 单张发票开票限额-->
          <input type="hidden" name="MAKE_MAX_SINGLE" value="<%=(int)(jqxx.getDzkpxe()*100)%>" />
          <!-- 月累计最高开票限额-->
          <input type="hidden" name="MAKE_MAX_SUM" value="<%=(int)(jqxx.getYljkpxe()*100)%>" />
          <!-- 月累计最高退票限额-->
          <input type="hidden" name="BACK_MAX_SUM" value="<%=(int)(jqxx.getYljtpxe()*100)%>" />
          <!-- 明细申报标志-->
          <input type="hidden" name="DECLARE_MAKE_TYPE" value="<%=jqxx.getMxsbbz() %>" />
          <!-- 申报类型-->
          <input type="hidden" name="DECLARE_TYPE" value="<%=jqxx.getSbfs() %>" />
          <!-- 税控卡有效时间-->
          <input type="hidden" name="F_START_DATE" value="<%=Util.toxkrq(jqxx.getKqyrq()) %>" />
          <!-- 税控卡截止时间-->
          <input type="hidden" name="F_END_DATE" value="<%=Util.toxkrq(jqxx.getKyxrq()) %>" />
          <!-- 主管科所-->
          <input type="hidden" name="ORGAN_CODE" value="<%=swjgbm %>" />
          <!-- 纳税人识别号-->
          <input type="hidden" name="TAXPAYER_CODE" value="<%=nsrxx.getNsrsbh() %>" />
          <!-- 下次申报月-->
          <input type="hidden" name="MAKE_END_DATE_MONTH" value="<%=SYSTEM.ENDMONTH %>" />
          <!-- 下次申报日-->
          <input type="hidden" name="MAKE_END_DATE_DAY" value="<%=SYSTEM.ENDDAY %>" />
          <%
		int i=0;
		Iterator it = jqszsm.iterator();
		while(it.hasNext()){
		...
		%>
          <input type="hidden" name="tax<%=i%>" value="<%=smStr%>" />
          <%
			i++;
		}
		if(i<6){
			...
			String val = "";
		%>
          <input type="hidden" name="tax<%=j%>" value="<%=val%>" />
          <%
			}
		}
		%>
          <input type="button" id="fh" name="btn3" onClick="window.location.href='/javaskweb/taxpayerServlet?method=skzzxx&jqbh=<%=jqxx.getJqbh() %>&nsrwjbm=<%=nsrxx.getNsrwjbm() %>&jumpFlag=<%=jumpFlag %>';" value="返 回" style="cursor: hand;" />
          &nbsp;&nbsp;
          ...
          <input type="button" id="jxtjjq" name="btn2" value=" 继续添加机器 " style="cursor: hand;" onclick="window.location.href='/javaskweb/taxpayerServlet?method=skzzxx&nsrwjbm=<%=nsrxx.getNsrwjbm() %>&jumpFlag=<%=jumpFlag %>';" />
          &nbsp;&nbsp; </td>
      </tr>
    </table>
