...
function add(){
	...
	//}
		...
		if(validator(frm)){
			...
			frm.action='/javaskweb/taxpayer.do?op=skzzxxUpdate';
			frm.submit();
		}
	}
}
...
%>
          ...
          <td align="center"><input id="skkh" name="skkh" type="text" maxlength="16" value="<%=jqxx.getSkkh() %>" required="required" dataType="integer" controlName="税控卡号" />
            &nbsp;<font color="#FF0000">*</font></td>
          <td align="cneter"><input type="radio" name="xgsm" value="1" id="yes">是 &nbsp;&nbsp;<input type="radio" name="xgsm" value=0 checked="checked" id="no">否</td>  </td>
        </tr>
      </table>
     <input type="hidden" value=<%=jqxx.getXgsmmc() %> id="jqxgsmmc"/>
     <script LANGUAGE="JavaScript" type="text/javascript">
          var xg=document.getElementById("jqxgsmmc").value;
...
%>
        <tr>
          ...
          <td align="center"><input type="checkbox" name="abc" value="<%=(String)hm.get("smbm") %>" onclick="checkPZ('<%=(String)hm.get("fpbm") %>',this)" <% if(jqszsmStr.indexOf((String)hm.get("smbm"))>=0){out.println("checked");} %> /></td>
        </tr>
        <%
	}
}
      ...
      <table width="100%" border="0" cellspacing="0" cellpadding="0" id="tableListBottom">
        <tr>
          <td height="40" align="center"><input type="hidden" name="nsrwjbm" value="<%=nsrwjbm %>"/>
            ...
            <input type="hidden" name="oldJqbh" value="<%=jqxx.getJqbh() %>"/>
            <input type="hidden" name="jqszsmStr" id="jqszsmStr" value="<%=jqxx.getJqszsmStr() %>"/>
