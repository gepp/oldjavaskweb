...
<div id="right">
  <div id="tool">
    <table width="100%" border="0" cellspacing="0" cellpadding="0" id="tableListTitle" style="padding-top:14px;">
      <tr>
      	<td>&nbsp;&nbsp;系统管理 &gt;&gt;&nbsp;&nbsp;<a href="/javaskweb/system.do?op=toRoleList" class="nav">权限管理</a>&gt;&gt;&nbsp;&nbsp;修改</td>
      </tr>
    </table>
  </div>
  <div id="main">
    <form name="form1" action="/javaskweb/system.do?op=roleEditAction" method="post" onSubmit="add();return validator(this)">
      <table width="100%" border="0" cellspacing="0" cellpadding="0" id="tableForm">
        <tr>
          <td width="15%">角色名称：</td>
          <td><input type="text" style="width:30%" name="rolename" value="<%=role.getRolename() %>" required="required" controlName="角色名称" /></td>
        </tr>
        <tr>
          <td>角色说明：</td>
          <td><textarea name="note" id="note" cols="45" rows="5"><%=role.getNote() %></textarea> </td>
        </tr>
      </table>
...
<%
ArrayList alMenu = role.getAlMenu();
...
%>
        <input type="checkbox" id="abc" name="abc" value="<%=(String)hm.get("menucode") %>" <% if(role.getMenuStr().indexOf((String)hm.get("menucode")+",")>0){out.println("checked");} %> />
<%
}
%>
&nbsp;
      ...
      <table width="100%" border="0" cellspacing="0" cellpadding="0" id="tableFormFooter">
        <tr>
          <td height="40" align="center">
            ...
            <input type="hidden" name="sid" value="<%=role.getSid() %>" />
