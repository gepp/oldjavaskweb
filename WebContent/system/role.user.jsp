...
function add(){
	...
	if(usernameStr==''){
		...
		window.location.href = '/javaskweb/system.do?op=roleAddUserAction&usernameStr='+usernameStr+'&sid='+sid;
	}
}
</script>
</head>
...
<div id="right">
  <div id="tool">
    <table width="100%" border="0" cellspacing="0" cellpadding="0" id="tableListTitle" style="padding-top:14px;">
      <tr>
      	<td style="font-size:16px; font-weight:800;">&nbsp;&nbsp;系统管理 &gt;&gt;&nbsp;&nbsp;<a href="/javaskweb/roleServlet?method=list" class="nav">权限管理</a>&gt;&gt;&nbsp;&nbsp;用户分配</td>
        <td align="right">
        <input type="hidden" id="sid" name="sid" value="<%=role.getSid() %>"/>
        <input type="button" name="btn" value=" 确 定 " onclick="add();" style="cursor:hand;" />&nbsp;&nbsp;
        <input type="button" name="btn" value=" 返 回 " style="cursor:hand;" onclick="window.location.href='/javaskweb/roleServlet?method=list';" />&nbsp;&nbsp;&nbsp;&nbsp;</td>
      </tr>
    </table>
  </div>
  <div id="main">
...
%>
      <tr>
        <td><input type="checkbox" id="a<%=user.getSid() %>" name="a<%=user.getSid() %>" value="<%=user.getUsername() %>" <% if(roleuser.indexOf(user.getUsername()+",")>0){out.println("checked");} %> /></td>
