...
function addZg(){
	window.location.href='/javaskweb/swjgServlet?method=getFromZG';
}

function add(){
	window.location.href='/javaskweb/system.do?op=toSwjgAdd';
}

function edit(swjgbm){
	window.location.href='/javaskweb/system.do?op=toSwjgEdit&swjgbm='+swjgbm;
}

function del(sid){
	if(confirm('确定删除该税务机关？')){
		window.location.href='/javaskweb/system.do?op=deleteSwjg&sid='+sid;
	}
}
</script>
</head>
...
<div id="right">
  <div id="tool">
    <table width="100%" border="0" cellspacing="0" cellpadding="0" id="tableListTitle" style="padding-top:14px;">
      <tr>
      	<td>&nbsp;&nbsp;系统管理 &gt;&gt;&nbsp;&nbsp;<a href="#" class="nav">税务机关管理</a></td>
...
%>
      <tr>
        ...
        <td><%=swjg.getStatus()==1?"启用":"未启用" %>&nbsp;</td>
        <td><input type="button" name="btn" value="修 改" style="cursor:hand;" onclick="edit('<%=swjg.getSwjgbm() %>');" />&nbsp;<!--<input type="button" style="cursor:hand;" name="btn" value="删 除" onclick="del(<%=swjg.getSid() %>);" />--></td>
      </tr>
<%
	}
}
